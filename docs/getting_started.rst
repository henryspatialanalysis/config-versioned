...
This guide introduces the **versioning** package, which simplifies management of
project settings and file I/O by combining them in a single :class:`~versioning.Config`
object.
...
have their own respective versions. The :class:`~versioning.Config` class makes it easy
...
.. code-block:: python
    ...
    example_config_path = str(r.files("versioning") / "data" / "example_config.yaml")
...
Create a :class:`~versioning.Config` object by passing either a path to a YAML file
...
:meth:`~versioning.Config.get` is preferable — it raises a clear ``KeyError`` if a
...
:class:`~versioning.Config` object its versioning capability. Each entry under
...
Use :meth:`~versioning.Config.get_dir_path` and
:meth:`~versioning.Config.get_file_path` to build these paths:
...
:meth:`~versioning.Config.read` and :meth:`~versioning.Config.write` to move data
through the pipeline:

.. code-block:: python

    # Copy the example input file into the raw_data directory
    example_csv = str(r.files("versioning") / "data" / "example_input_file.csv")
...
These methods delegate to :func:`~versioning.autoread` and
:func:`~versioning.autowrite`, which dispatch on file extension. To see every
supported extension:

.. code-block:: python

    from versioning import get_file_reading_functions, get_file_writing_functions
...
:meth:`~versioning.Config.write_self` writes the current config dict as
...
constructing a :class:`~versioning.Config` object. This sets or overwrites the
